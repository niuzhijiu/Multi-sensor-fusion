...
std::vector<CamData> loadCamCsv(const std::string& path) {
    ...
    while (std::getline(file, line)) {
        ...
        if (!ts.empty() && !fname.empty()) {
            ...
            //将数据添加到存储器保存
            data.push_back(d);
            valid_data_count++;  // 新增：统计有效数据行数
        }
    }

    //关闭文件流（规范资源释放）
    file.close();
    //打印CSV加载结果（便于确认数据是否正常读取）
    ROS_INFO("Camera CSV loaded: Path=%s, Valid data count=%d, Total lines processed=%d", 
             path.c_str(), valid_data_count, (valid_data_count + 1));  // +1 包含表头行
...
int main(int argc, char** argv) {
    ...
    nh.param<std::string>("cam_csv", cam_csv_path, "/home/cat/20251029_test/cam0_aligned.csv");
    nh.param<std::string>("image_dir", image_dir, "/home/cat/euroc_bag_split/MH_01_easy_split/cam0/");
    nh.param<std::string>("imu_csv", imu_csv_path, "/home/cat/euroc_bag_split/MH_01_easy_split/imu0_data.csv");
    ...
    ROS_INFO("IMU CSV load: %s", imu_csv_path.c_str());  // 建议补充IMU路径打印
